...
Cross-platform (Linux + macOS) PHP extension: Zephir static classes under flat `Posi\` (`System`, `Memory`, `Termios`) that call a thin C ABI in `src/*-api.{c,h}`, which wraps libc POSIX (`open`, `read`, `ioctl`, `termios`, …). File descriptors and native buffer pointers are PHP `int`s. Windows is excluded. No FFI. Version **0.9.0** (0.9 line, paired with ext-ftdi 0.9.0; prior lines 0.8.0, 0.7.0, 0.5.0).
...
* [IDE stub path lag](traps/ide-stub-path-lag.md) - `ide/0.9.0` is regenerated only alongside a version bump
